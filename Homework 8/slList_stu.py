...
def insertListAt(lst1, idx, lst2):
    ...
    """
    1-go to target location in lst1
    2-iterate through lst2, creating a new node (popping from lst1)
    3-redo last connection
    ...
    while cursor1 != None:
        if index_counter==idx-1:
            while cursor2.next != None:
                cursor1.next = Node(cursor2.data, cursor1.next)
...
def copySegment(lst, idx, segSize):
    ...
    cursor1 = lst.head
    while cursor1 != None:
        if index_counter>=idx and index_counter <idx+segSize:
            append(lst2,cursor1.data)
        index_counter+=1
        cursor1=cursor1.next
    insertListAt(lst, idx+segSize, lst2)